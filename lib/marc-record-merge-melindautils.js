...
;	} else if(typeof exports === 'object') {
	...
	function constructor(config) {
		...
		var asteri = new AlephXServices(config.asteri);
		var libtest = new AlephXServices(config.libtest);
		...
		function canMerge(otherRecord, preferredRecord) {
			...
			[otherRecord, preferredRecord].forEach(function(record, i) {
				var isDeletedBySTA_Field = record.fields.filter(byTag('STA')).map(toFirstSubfieldValue('a')).some(function(sta_a) { 
					return sta_a === "DELETED";
				});

				if (record.leader.substr(5,1) === 'd' || isDeletedBySTA_Field) {
					var id = record.fields.filter(byTag('001'));
					id = id[0] || "unknown";
					id = id.value || "unknown";

					errors.push("record is deleted: " + id);
				}
			});
			...
			validateFields({
				...
				console.log("STAT 245] " + msg);
			});

			reportDiacritics(['245'], otherRecord, preferredRecord).forEach(function(msg) {
				console.log("STAT DIAC] " + msg);
			});
		...
		function isHostRecord(record) {
			...
			return libtest.raw({
				...
				throw new Error("Could not parse response from X-server on isHostRecord validation");
		...
		function applyPostMergeModifications(otherRecord, preferredRecord, mergedRecord) {
			...
			//  
			console.log("applyPostMergeModifications");
			...
			reprintFields.map(function(field) {
				...
				if ( otherQueryObject === undefined || prefQueryObject === undefined || 
					...
					if (otherQueryObject.tag !== prefQueryObject.tag) {
						...
						Q.all(queries).then(function(results) {
							...
							console.log("preferredIsAuthorized:", preferredIsAuthorized, "otherIsAuthorized:", otherIsAuthorized);
		...
		function reportDiacritics(tagList, otherRecord, preferredRecord) {
			var issues = [];
			...
			Object.keys(validationConfig).forEach(function(tag) {
				...
				if (opts.validate.indexOf("preferredIsSuperset") !== -1) {
					...
					if (preferred_fields.length > 0 &&
						...
						errors.push("Preferred record's has stopfields and they are a subset of the other record's in field: " + tag);	
					}
					...
					if (other_fields.length > 0 && preferred_fields.length > 0) {
