...
(function(root, factory) {
	...
	if (typeof define === 'function' && define.amd) {
		...
		module.exports = factory(require('q'), require('aleph-x-query'), require('xregexp').XRegExp);  // jshint ignore:line
	} else {
		root.merge = factory(root.Q, root.AlephXServices);
	}
}(this, function(Q, AlephXServices, XRegExp) {
	"use strict";
	...
	function constructor(config) {
		...
		function canMerge(otherRecord, preferredRecord) {
			...
			var otherRecordType = inferTypeAndCount(selectValue(otherRecord, '300', 'a'));
			var preferredRecordType = inferTypeAndCount(selectValue(preferredRecord, '300', 'a'));
		...
		function inferTypeAndCount(str) {
			...
			var IMPLICIT_VOLUME_WITHOUT_PREFACE = /^[0-9]* pages$/;
		...
		function isHostRecord(record) {
			...
			if (id === undefined) {
				var deferred = Q.defer();
				deferred.reject(new Error("Could not parse record id"));
				return deferred.promise;
			}
